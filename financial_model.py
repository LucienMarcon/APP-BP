...
class Financing:
    ...
    def calculate_equity(self, total_capex):
        # Excel Logic: Equity (B4) = CAPEX_Summary!B6 - Debt (B5)
        return total_capex - self.debt_principal
...
class CashflowEngine:
    def __init__(self, general: General, construction: Construction, financing: Financing, capex_summary: CapexSummary, operation: OperationExit, amortization: Amortization, scheduler: Scheduler):
        ...
        noi_n_plus_1 = (rent_n - (opex_fixed_n + opex_var_n)) * (1 + operation.rent_growth)
        ...
        for y in years:
            ...
            if y == 0:
                ...
                sales = scheduler.sale_schedule.get(y, 0) + (net_exit_val if y == operation.holding_period else 0)
                opex = (general.gfa * operation.opex_per_m2 * ((1 + operation.inflation) ** (y - 1))) + (rent * operation.pm_fee_pct)
                noi = rent + sales - opex
                
                row['Rental Income'] = rent; row['Sales Income'] = sales; row['OPEX'] = -opex; row['NOI'] = noi
                
                row['CAPEX'] = -construction.get_yearly_capex().get(y, 0)
                
                debt = amortization.schedule.get(y, {'payment': 0, 'closing': 0, 'interest': 0})
                ...
                row['Upfront Fees'] = 0; row['Debt Drawdown'] = 0
        ...
        # Handle Debt Drawdown in Y1 (Simplified assumption: Debt covers construction start)
        ...
        # Calculate KPIs using Total Equity needed derived from CAPEX Summary
