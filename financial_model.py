...
class Amortization:
    """
    Reconstructs 'Amortization' sheet logic exactly as described.
    Logic:
    - Years 1 to Exit Year (usually 20).
    - Grace Period (Interest Only).
    - Amortization Phase (Constant Annuity).
    - Cut at Exit Year.
    """
    def __init__(self, financing: Financing, operation: OperationExit):
        self.schedule = {}
        
        loan_amount = financing.debt_principal
        annual_rate = financing.interest_rate
        term_years = financing.loan_term
        grace_years = financing.grace_period
        exit_year = operation.holding_period
        
        # PMT Calculation (Standard Excel PMT equivalent)
        # PMT is calculated on the remaining term (Term - Grace)
        amortization_duration = term_years - grace_years
        
        if amortization_duration > 0:
            # Numpy PMT formula: rate, nper, pv, fv=0, type=0
            annuity = npf.pmt(annual_rate, amortization_duration, -loan_amount)
        else:
            annuity = 0

        # Initialize Year 0
        opening_balance = loan_amount # At start of Year 1 (end of Year 0 logic often implies Drawdown)
        # But schedule usually runs Y1..20. Let's track Balance at START of year.
        
        current_balance = loan_amount

        # Loop from Year 1 to Term (or Exit)
        # Excel logic: Row 14 is Year 0 (Opening 0). Row 15 is Year 1 (Opening = Debt).
        # Let's simulate Year 1 to 20+
        
        for year in range(1, term_years + 5): # Go a bit further to be safe
            
            if year > exit_year:
                # After exit, everything is 0
                self.schedule[year] = {
                    'opening': 0, 'payment': 0, 'interest': 0, 'principal': 0, 'closing': 0
                }
                continue
            ...
            # Payment & Principal Logic
            if year <= grace_years:
                # Grace Period: Interest Only
                principal = 0
                payment = interest
            elif year <= term_years:
                # Amortization Phase
                payment = annuity
            ...
            # Logic check: If closing < 0 (rounding errors), set to 0
    ...
    def __init__(self, 
        ...
        for y in years:
            ...
            if y == 0:
                ...
                # Net Cash Flow (Unlevered Logic + Financing)
        ...
        # Equity Drawdown (Y1 adjustment)
        if 1 in self.df.index:
            # self.df.at[1, 'Debt Drawdown'] logic handled in loop
            pass
